...
public class MainController {
	...
	public void getSynonymTest() {
		try {
			...
			/*
			 * for (Tag s : tags) { HttpResponse<JsonNode> response = Unirest
			 * .get("https://wordsapiv1.p.mashape.com/words/" + s.getTag() +
			 * "/synonyms") .header("X-Mashape-Key",
			 * "UCoDyzYDh1mshy874KnxdaKo8Ae2p1qWHK9jsnHu66tUeO7oPs")
			 * .header("Accept", "application/json").asJson(); Gson gson = new
			 * Gson(); SynonymResponse synonymResponse =
			 * gson.fromJson(response.getBody().toString(),
			 * SynonymResponse.class); synonymResponse.setTag(s);
	...
	public ModelAndView loginAdmin(@RequestParam("username") String username,
		...
		if (username != null && password != null) {
			Message message;
				message = userService.authenticateUser(user, "admin");
			if (message.getFlag()) {
